...
def main():
    ...
    while again == 'y':
        again = get_response()
        #the user is asked if they want to roll again using get_response function
        if total_user_sum < 21 and again == 'y':
...
##                again = 'c'
    ...
    if again == 'n':
        ...
        #If the user hit or exceeded 21, the user would not have been asked
        #whether they wanted to roll again.  So in the case where the computer
        #and user sums are not equal and neither are >=21, the higher sum wins
        #user wins if user's sum is higher
        if total_user_sum > total_computer_sum:
            print('User\'s points:', total_user_sum)
            print('Computer\'s points:', total_computer_sum)
            print('User wins')
            
        #computer wins if computer's sum is higher  
        elif total_user_sum < total_computer_sum:
            print('User\'s points:', total_user_sum)
            print('Computer\'s points:', total_computer_sum)
            print('Computer wins')
